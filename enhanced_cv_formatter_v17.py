...
class EnhancedCVFormatterV17:
    def __init__(self):
        self.template_path = os.path.join(os.path.dirname(__file__), 'mawney_cv_template_force_pages_v17.html')
        
    def format_cv_with_template(self, cv_content, filename):
        """Format CV using forced pagination template"""
        try:
            ...
            # Format sections with better parsing
            work_exp = self._format_work_experience_v17(cv_data.get('work_experience', []))
            education = self._format_education_v17(cv_data.get('education', []))
            languages = self._format_languages_v17(cv_data.get('languages', []))
            computer_skills = self._format_computer_skills_v17(cv_data.get('computer_skills', []))
            extra_curricular = self._format_extra_curricular_v17(cv_data.get('extra_curricular', []))
    ...
    def _parse_cv_data(self, text):
        ...
        if work_section:
            cv_data['work_experience'] = self._parse_work_experience_v17(work_section)
        else:
            # Try to find work experience without section header
            cv_data['work_experience'] = self._parse_work_experience_v17(text)
        
        # Parse education - improved
        education_section = self._extract_section(text, ['EDUCATION', 'ACADEMIC BACKGROUND', 'QUALIFICATIONS'])
        if education_section:
            cv_data['education'] = self._parse_education_v17(education_section)
        else:
            cv_data['education'] = self._parse_education_v17(text)
        
        # Parse languages
        languages_section = self._extract_section(text, ['LANGUAGES', 'LANGUAGE SKILLS'])
        if languages_section:
            cv_data['languages'] = self._parse_languages_v17(languages_section)
        ...
        if skills_section:
            cv_data['computer_skills'] = self._parse_computer_skills_v17(skills_section)
        
        # Parse extra curricular
        extra_section = self._extract_section(text, ['EXTRA CURRICULAR', 'ACTIVITIES', 'INTERESTS', 'HOBBIES'])
        if extra_section:
            cv_data['extra_curricular'] = self._parse_extra_curricular_v17(extra_section)
    ...
    def _parse_work_experience_v17(self, section):
        """Improved work experience parsing"""
        entries = []
    ...
    def _parse_education_v17(self, section):
    ...
    def _parse_languages_v17(self, section):
        """Parse languages"""
        languages = []
    ...
    def _parse_computer_skills_v17(self, section):
        """Parse computer skills"""
        skills = []
    ...
    def _parse_extra_curricular_v17(self, section):
    ...
    def _format_work_experience_v17(self, work_exp):
        """Format work experience with proper layout"""
        if not work_exp:
    ...
    def _format_education_v17(self, education):
        """Format education with proper layout"""
        if not education:
    ...
    def _format_languages_v17(self, languages):
    ...
    def _format_computer_skills_v17(self, skills):
    ...
    def _format_extra_curricular_v17(self, activities):
