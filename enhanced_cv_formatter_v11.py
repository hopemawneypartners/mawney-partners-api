...
class EnhancedCVFormatterV11:
    def __init__(self):
        self.template_path = "mawney_cv_template_viewport_fixed.html"
        self.use_ai_parsing = False  # Using rule-based parsing only
        
    def format_cv_with_template(self, cv_content, filename):
        """Format CV using the viewport-fixed template with improved parsing"""
        try:
            # Clean and parse the CV content
            parsed_data = self._parse_cv_v11(cv_content)
    ...
    def _parse_cv_v11(self, cv_content):
        """Enhanced parsing with focus on removing phone numbers and fixing concatenated words"""
        # Clean the CV text aggressively
        cleaned_text = self._clean_cv_text_v11(cv_content)
        
        # Parse different sections
        parsed_data = {
            "name": self._extract_name_v11(cleaned_text),
            "email": self._extract_email(cleaned_text),
            "location": self._extract_location(cleaned_text),
            "work_experience": self._parse_work_experience_v11(cleaned_text),
            "education": self._parse_education_v11(cleaned_text),
            "languages": self._parse_languages(cleaned_text),
            "computer_skills": self._parse_computer_skills_v11(cleaned_text),
            "extra_curricular": self._parse_extra_curricular(cleaned_text)
        }
        
        return parsed_data
    
    def _clean_cv_text_v11(self, text):
        """Aggressive text cleaning to fix concatenated words and remove phone numbers"""
        if not text:
            return ""
    ...
    def _extract_name_v11(self, text):
    ...
    def _parse_work_experience_v11(self, text):
    ...
    def _parse_education_v11(self, text):
    ...
    def _parse_computer_skills_v11(self, text):
...
# Create instance
enhanced_cv_formatter_v11 = EnhancedCVFormatterV11()
