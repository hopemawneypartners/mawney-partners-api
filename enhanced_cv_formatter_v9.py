#!/usr/bin/env python3
"""
Enhanced CV Formatter V9 for Mawney Partners
Focused specifically on fixing pagination and viewport issues
"""
...
class EnhancedCVFormatterV9:
    """Enhanced CV formatter V9 - Focused on pagination and viewport fixes"""
    
    def __init__(self):
        # Use the new pagination-focused template first
    ...
    def format_cv_with_template(self, cv_data: str, filename: str = '') -> Dict[str, Any]:
        """Format CV using the pagination-focused template"""
        try:
            logger.info(f"=== ENHANCED CV FORMATTING V9 STARTED ===")
            logger.info(f"Filename: {filename}")
            logger.info(f"Raw CV data length: {len(cv_data)} characters")
            
            # Step 1: Complete text cleaning and structuring
            structured_cv = self._complete_text_cleaning_v9(cv_data)
            logger.info(f"Cleaned CV data length: {len(structured_cv)} characters")
            
            # Step 2: Parse CV with V9 logic
            parsed_data = self._parse_cv_v9(structured_cv)
            ...
            logger.info(f"=== ENHANCED CV FORMATTING V9 COMPLETED ===")
            
            return {
                'success': True,
                'html_version': formatted_html,
                'text_version': self._extract_text_from_html(formatted_html),
                'analysis': f"CV formatted using Mawney Partners template V9 (pagination-focused). Extracted: {len(parsed_data.get('experience', []))} experience items, {len(parsed_data.get('education', []))} education items.",
            ...
            logger.error(f"Error in enhanced CV formatting V9: {e}", exc_info=True)
            return {
                'success': False,
                'error': str(e),
                'html_version': '',
    ...
    def _complete_text_cleaning_v9(self, text: str) -> str:
    ...
    def _parse_cv_v9(self, cv_text: str) -> Dict[str, Any]:
        """Parse CV with V9 logic - focused on clean parsing"""
        logger.info("Using CV parsing V9 - focused on clean parsing...")
        ...
        # Parse sections with V9 logic
        parsed['experience'] = self._extract_experience_v9(lines)
        parsed['education'] = self._extract_education_v9(lines)
        parsed['languages'], parsed['computer_skills'] = self._extract_skills_v9(lines)
        parsed['extra_curricular'] = self._extract_extra_curricular_v9(lines)
        
        return parsed
    
    def _extract_experience_v9(self, lines: List[str]) -> List[Dict[str, Any]]:
        """Extract experience with V9 logic - clean extraction"""
        experience = []
        ...
        while i < len(lines):
            ...
            if in_experience_section and line:
                ...
                if self._is_company_line_v9(line):
                ...
                elif current_experience and self._is_job_title_line_v9(line):
                    # This is a job title
                    current_experience['title'] = line
                elif current_experience and self._is_date_line_v9(line):
                    # This contains dates - EXTRACT THEM PROPERLY
                    dates = self._extract_dates_from_line_v9(line)
                    if dates:
        ...
        for exp in experience:
            exp['responsibilities'] = self._rewrite_responsibilities_v9(exp.get('responsibilities', []))
        
        return experience
    
    def _is_company_line_v9(self, line: str) -> bool:
        """Check if line is a company name with V9 logic"""
    ...
    def _is_job_title_line_v9(self, line: str) -> bool:
        """Check if line is a job title with V9 logic"""
    ...
    def _is_date_line_v9(self, line: str) -> bool:
        """Check if line contains dates with V9 logic"""
    ...
    def _extract_dates_from_line_v9(self, line: str) -> str:
        """Extract dates from a line with V9 logic"""
    ...
    def _rewrite_responsibilities_v9(self, responsibilities: List[str]) -> List[str]:
        """Rewrite and clean up responsibilities with V9 logic"""
        rewritten = []
    ...
    def _extract_education_v9(self, lines: List[str]) -> List[Dict[str, Any]]:
        """Extract education with V9 logic - clean extraction"""
        education = []
    ...
    def _extract_skills_v9(self, lines: List[str]) -> tuple[List[str], List[str]]:
        """Extract languages and computer skills with V9 logic - clean extraction"""
        languages = []
        computer_skills = []
    ...
    def _extract_extra_curricular_v9(self, lines: List[str]) -> List[str]:
        """Extract extra curricular activities with V9 logic"""
        activities = []
    ...
    def _fill_template_with_data(self, template: str, data: Dict[str, Any], top_logo: str, bottom_logo: str) -> str:
        ...
        html = html.replace('{EXPERIENCE_ITEMS}', self._format_experience_items_v9(data))
        html = html.replace('{EDUCATION_ITEMS}', self._format_education_items_v9(data))
        html = html.replace('{SKILLS_LIST}', self._format_skills_v9_template(data))
    ...
    def _format_experience_items_v9(self, data: Dict[str, Any]) -> str:
        """Format experience items with V9 layout - DATES ON RIGHT SIDE"""
        items = []
    ...
    def _format_education_items_v9(self, data: Dict[str, Any]) -> str:
        """Format education items with V9 layout"""
        items = []
    ...
    def _format_skills_v9_template(self, data: Dict[str, Any]) -> str:
