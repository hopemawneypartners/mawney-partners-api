...
class EnhancedCVFormatterV18:
    def __init__(self):
        self.template_path = os.path.join(os.path.dirname(__file__), 'mawney_cv_template_aggressive_pagination_v18.html')
        
    def format_cv_with_template(self, cv_content, filename):
        """Format CV using aggressive pagination template"""
        try:
            ...
            # Format sections (keeping parsing exactly the same as V17)
            work_exp = self._format_work_experience_v18(cv_data.get('work_experience', []))
            education = self._format_education_v18(cv_data.get('education', []))
            languages = self._format_languages_v18(cv_data.get('languages', []))
            computer_skills = self._format_computer_skills_v18(cv_data.get('computer_skills', []))
            extra_curricular = self._format_extra_curricular_v18(cv_data.get('extra_curricular', []))
            ...
            return {
                ...
                'version': 'V18_AggressivePagination'
            }
            ...
            return {
                ...
                'filename': filename,
                'error': str(e),
                'version': 'V18_AggressivePagination_Error'
            }
    ...
    def _parse_cv_data(self, text):
        ...
        if work_section:
            cv_data['work_experience'] = self._parse_work_experience_v18(work_section)
        else:
            # Try to find work experience without section header
            cv_data['work_experience'] = self._parse_work_experience_v18(text)
        
        # Parse education - improved
        education_section = self._extract_section(text, ['EDUCATION', 'ACADEMIC BACKGROUND', 'QUALIFICATIONS'])
        if education_section:
            cv_data['education'] = self._parse_education_v18(education_section)
        else:
            cv_data['education'] = self._parse_education_v18(text)
        
        # Parse languages
        languages_section = self._extract_section(text, ['LANGUAGES', 'LANGUAGE SKILLS'])
        if languages_section:
            cv_data['languages'] = self._parse_languages_v18(languages_section)
        ...
        if skills_section:
            cv_data['computer_skills'] = self._parse_computer_skills_v18(skills_section)
        
        # Parse extra curricular
        extra_section = self._extract_section(text, ['EXTRA CURRICULAR', 'ACTIVITIES', 'INTERESTS', 'HOBBIES'])
        if extra_section:
            cv_data['extra_curricular'] = self._parse_extra_curricular_v18(extra_section)
    ...
    def _parse_work_experience_v18(self, section):
        """Improved work experience parsing - same as V17"""
        entries = []
    ...
    def _parse_education_v18(self, section):
        """Improved education parsing - same as V17"""
        entries = []
    ...
    def _parse_languages_v18(self, section):
        """Parse languages - same as V17"""
        languages = []
    ...
    def _parse_computer_skills_v18(self, section):
        """Parse computer skills - same as V17"""
        skills = []
    ...
    def _parse_extra_curricular_v18(self, section):
        """Parse extra curricular activities - same as V17"""
        activities = []
    ...
    def _format_work_experience_v18(self, work_exp):
    ...
    def _format_education_v18(self, education):
    ...
    def _format_languages_v18(self, languages):
    ...
    def _format_computer_skills_v18(self, skills):
    ...
    def _format_extra_curricular_v18(self, activities):
...
# Create instance
enhanced_cv_formatter_v18 = EnhancedCVFormatterV18()
