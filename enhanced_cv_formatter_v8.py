#!/usr/bin/env python3
"""
Enhanced CV Formatter V8 for Mawney Partners
Final version to fix all parsing issues completely
"""
...
class EnhancedCVFormatterV8:
    """Enhanced CV formatter V8 - Final version to fix all parsing issues"""
    
    def __init__(self):
        # Use the new final template
        ...
        if os.path.exists(final_template):
            self.template_path = final_template
            logger.info("Using final CV template with proper layout")
    ...
    def format_cv_with_template(self, cv_data: str, filename: str = '') -> Dict[str, Any]:
        """Format CV using the final template with complete parsing fix"""
        try:
            logger.info(f"=== ENHANCED CV FORMATTING V8 STARTED ===")
            logger.info(f"Filename: {filename}")
            logger.info(f"Raw CV data length: {len(cv_data)} characters")
            
            # Step 1: Complete text cleaning and structuring
            structured_cv = self._complete_text_cleaning_v8(cv_data)
            logger.info(f"Cleaned CV data length: {len(structured_cv)} characters")
            
            # Step 2: Parse CV with V8 logic
            parsed_data = self._parse_cv_v8(structured_cv)
            ...
            logger.info(f"=== ENHANCED CV FORMATTING V8 COMPLETED ===")
            
            return {
                'success': True,
                'html_version': formatted_html,
                'text_version': self._extract_text_from_html(formatted_html),
                'analysis': f"CV formatted using Mawney Partners template V8. Extracted: {len(parsed_data.get('experience', []))} experience items, {len(parsed_data.get('education', []))} education items.",
            ...
            logger.error(f"Error in enhanced CV formatting V8: {e}", exc_info=True)
            return {
                'success': False,
                'error': str(e),
                'html_version': '',
    ...
    def _complete_text_cleaning_v8(self, text: str) -> str:
    ...
    def _parse_cv_v8(self, cv_text: str) -> Dict[str, Any]:
        """Parse CV with V8 logic - completely rewritten parsing"""
        logger.info("Using CV parsing V8 - completely rewritten parsing...")
        ...
        # Parse sections with V8 logic
        parsed['experience'] = self._extract_experience_v8(lines)
        parsed['education'] = self._extract_education_v8(lines)
        parsed['languages'], parsed['computer_skills'] = self._extract_skills_v8(lines)
        parsed['extra_curricular'] = self._extract_extra_curricular_v8(lines)
        
        return parsed
    
    def _extract_experience_v8(self, lines: List[str]) -> List[Dict[str, Any]]:
        """Extract experience with V8 logic - completely rewritten"""
        experience = []
        ...
        while i < len(lines):
            ...
            if in_experience_section and line:
                ...
                if self._is_company_line_v8(line):
                ...
                elif current_experience and self._is_job_title_line_v8(line):
                    # This is a job title
                    current_experience['title'] = line
                elif current_experience and self._is_date_line_v8(line):
                    # This contains dates - EXTRACT THEM PROPERLY
                    dates = self._extract_dates_from_line_v8(line)
                    if dates:
        ...
        for exp in experience:
            exp['responsibilities'] = self._rewrite_responsibilities_v8(exp.get('responsibilities', []))
        
        return experience
    
    def _is_company_line_v8(self, line: str) -> bool:
        """Check if line is a company name with V8 logic"""
    ...
    def _is_job_title_line_v8(self, line: str) -> bool:
        """Check if line is a job title with V8 logic"""
    ...
    def _is_date_line_v8(self, line: str) -> bool:
        """Check if line contains dates with V8 logic"""
    ...
    def _extract_dates_from_line_v8(self, line: str) -> str:
        """Extract dates from a line with V8 logic"""
    ...
    def _rewrite_responsibilities_v8(self, responsibilities: List[str]) -> List[str]:
        """Rewrite and clean up responsibilities with V8 logic"""
        rewritten = []
    ...
    def _extract_education_v8(self, lines: List[str]) -> List[Dict[str, Any]]:
        """Extract education with V8 logic - completely rewritten"""
        education = []
    ...
    def _extract_skills_v8(self, lines: List[str]) -> tuple[List[str], List[str]]:
        """Extract languages and computer skills with V8 logic - completely rewritten"""
        languages = []
        computer_skills = []
        ...
        if not languages and not computer_skills and in_skills_section:
            ...
            # Try to categorize them properly - EXCLUDE PERSONAL INFO
            for line in skills_lines:
                clean_line = line.strip()
                if len(clean_line) > 10:
                    # EXCLUDE personal information
                    if any(exclude in clean_line.lower() for exclude in ['ting qin', 'frm', 'moorland way', 'maidenhead', 'tel:', 'email:', 'address', 'phone']):
                        continue
    ...
    def _extract_extra_curricular_v8(self, lines: List[str]) -> List[str]:
        """Extract extra curricular activities with V8 logic"""
        activities = []
    ...
    def _fill_template_with_data(self, template: str, data: Dict[str, Any], top_logo: str, bottom_logo: str) -> str:
        ...
        html = html.replace('{EXPERIENCE_ITEMS}', self._format_experience_items_v8(data))
        html = html.replace('{EDUCATION_ITEMS}', self._format_education_items_v8(data))
        html = html.replace('{SKILLS_LIST}', self._format_skills_v8_template(data))
    ...
    def _format_experience_items_v8(self, data: Dict[str, Any]) -> str:
        """Format experience items with V8 layout - DATES ON RIGHT SIDE"""
        items = []
    ...
    def _format_education_items_v8(self, data: Dict[str, Any]) -> str:
        """Format education items with V8 layout"""
        items = []
    ...
    def _format_skills_v8_template(self, data: Dict[str, Any]) -> str:
...
enhanced_cv_formatter_v8 = EnhancedCVFormatterV8()
