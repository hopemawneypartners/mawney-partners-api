#!/usr/bin/env python3
"""
Enhanced CV Formatter V3 for Mawney Partners
Completely rewritten to handle real CV structures and fix all parsing issues
"""
...
class EnhancedCVFormatterV3:
    """Enhanced CV formatter V3 with robust parsing for real CV structures"""
    ...
    def format_cv_with_template(self, cv_data: str, filename: str = '') -> Dict[str, Any]:
        """Format CV using the Mawney Partners template with robust parsing"""
        try:
            logger.info(f"=== ENHANCED CV FORMATTING V3 STARTED ===")
            logger.info(f"Filename: {filename}")
            logger.info(f"Raw CV data length: {len(cv_data)} characters")
            
            # Step 1: Clean and structure the CV text
            structured_cv = self._structure_cv_text(cv_data)
            logger.info(f"Structured CV data length: {len(structured_cv)} characters")
            
            # Step 2: Parse CV with robust logic for real CV structures
            parsed_data = self._parse_cv_robust(structured_cv)
            ...
            logger.info(f"=== ENHANCED CV FORMATTING V3 COMPLETED ===")
            
            return {
                'success': True,
                'html_version': formatted_html,
                'text_version': self._extract_text_from_html(formatted_html),
                'analysis': f"CV formatted using Mawney Partners template V3. Extracted: {len(parsed_data.get('experience', []))} experience items, {len(parsed_data.get('education', []))} education items.",
            ...
            logger.error(f"Error in enhanced CV formatting V3: {e}", exc_info=True)
            return {
                'success': False,
                'error': str(e),
                'html_version': '',
    ...
    def _structure_cv_text(self, text: str) -> str:
        """Structure CV text with proper line breaks and section separation"""
        import re
    ...
    def _parse_cv_robust(self, cv_text: str) -> Dict[str, Any]:
        """Parse CV with robust logic for real CV structures"""
        logger.info("Using robust CV parsing V3...")
        ...
        # Parse sections with robust logic
        parsed['experience'] = self._extract_experience_robust(lines)
        parsed['education'] = self._extract_education_robust(lines)
        parsed['languages'], parsed['computer_skills'] = self._extract_skills_robust(lines)
        parsed['extra_curricular'] = self._extract_extra_curricular_robust(lines)
        
        return parsed
    
    def _extract_experience_robust(self, lines: List[str]) -> List[Dict[str, Any]]:
        """Extract experience with robust logic for real CV structures"""
        experience = []
        ...
        for i, line in enumerate(lines):
            ...
            if in_experience_section and line:
                ...
                if self._is_company_or_job_line_robust(line):
                    # Save previous experience
                    if current_experience and (current_experience.get('company') or current_experience.get('title')):
                        experience.append(current_experience)
                    
                    # Start new experience
                    current_experience = self._parse_experience_line_robust(line)
        ...
        for exp in experience:
            exp['responsibilities'] = self._rewrite_responsibilities_robust(exp.get('responsibilities', []))
        
        return experience
    
    def _is_company_or_job_line_robust(self, line: str) -> bool:
        """Check if line is likely a company or job title with robust logic"""
        ...
        if re.search(r'\b(London|New York|Singapore|Hong Kong|Sydney|Paris|Frankfurt|Tokyo)\b', line, re.IGNORECASE):
            return True
        
        return False
    
    def _parse_experience_line_robust(self, line: str) -> Dict[str, Any]:
        """Parse a single experience line with robust logic"""
        # Extract dates
        ...
        # Extract location
        location_match = re.search(r'\b(London|New York|Singapore|Hong Kong|Sydney|Paris|Frankfurt|Tokyo)\b', clean_line, re.IGNORECASE)
        location = location_match.group(1) if location_match else ""
        
        # Remove location from the line
        if location:
            clean_line = clean_line.replace(location, '').strip()
    ...
    def _rewrite_responsibilities_robust(self, responsibilities: List[str]) -> List[str]:
        """Rewrite and clean up responsibilities with robust logic"""
        rewritten = []
    ...
    def _extract_education_robust(self, lines: List[str]) -> List[Dict[str, Any]]:
        """Extract education with robust logic"""
        education = []
    ...
    def _extract_skills_robust(self, lines: List[str]) -> tuple[List[str], List[str]]:
        """Extract languages and computer skills with robust logic"""
        languages = []
        computer_skills = []
    ...
    def _extract_extra_curricular_robust(self, lines: List[str]) -> List[str]:
        """Extract extra curricular activities with robust logic"""
        activities = []
    ...
    def _fill_template_with_data(self, template: str, data: Dict[str, Any], top_logo: str, bottom_logo: str) -> str:
        ...
        html = html.replace('{EXPERIENCE_ITEMS}', self._format_experience_items_robust(data))
        html = html.replace('{EDUCATION_ITEMS}', self._format_education_items_robust(data))
        html = html.replace('{SKILLS_LIST}', self._format_skills_robust_template(data))
    ...
    def _format_experience_items_robust(self, data: Dict[str, Any]) -> str:
        """Format experience items with robust layout"""
        items = []
    ...
    def _format_education_items_robust(self, data: Dict[str, Any]) -> str:
        """Format education items with robust layout"""
        items = []
    ...
    def _format_skills_robust_template(self, data: Dict[str, Any]) -> str:
...
enhanced_cv_formatter_v3 = EnhancedCVFormatterV3()
