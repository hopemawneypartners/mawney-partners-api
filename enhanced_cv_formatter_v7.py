#!/usr/bin/env python3
"""
Enhanced CV Formatter V7 for Mawney Partners
Complete rewrite to fix all fundamental parsing and display issues
"""
...
class EnhancedCVFormatterV7:
    """Enhanced CV formatter V7 - Complete rewrite to fix all issues"""
    ...
    def format_cv_with_template(self, cv_data: str, filename: str = '') -> Dict[str, Any]:
        """Format CV using the final template with complete rewrite"""
        try:
            logger.info(f"=== ENHANCED CV FORMATTING V7 STARTED ===")
            logger.info(f"Filename: {filename}")
            logger.info(f"Raw CV data length: {len(cv_data)} characters")
            
            # Step 1: Complete text cleaning and structuring
            structured_cv = self._complete_text_cleaning(cv_data)
            logger.info(f"Cleaned CV data length: {len(structured_cv)} characters")
            
            # Step 2: Parse CV with completely new logic
            parsed_data = self._parse_cv_v7(structured_cv)
            ...
            logger.info(f"=== ENHANCED CV FORMATTING V7 COMPLETED ===")
            
            return {
                'success': True,
                'html_version': formatted_html,
                'text_version': self._extract_text_from_html(formatted_html),
                'analysis': f"CV formatted using Mawney Partners template V7. Extracted: {len(parsed_data.get('experience', []))} experience items, {len(parsed_data.get('education', []))} education items.",
            ...
            logger.error(f"Error in enhanced CV formatting V7: {e}", exc_info=True)
            return {
                'success': False,
                'error': str(e),
                'html_version': '',
    ...
    def _complete_text_cleaning(self, text: str) -> str:
    ...
    def _parse_cv_v7(self, cv_text: str) -> Dict[str, Any]:
        """Parse CV with V7 logic - complete rewrite"""
        logger.info("Using CV parsing V7 - complete rewrite...")
        ...
        # Parse sections with V7 logic
        parsed['experience'] = self._extract_experience_v7(lines)
        parsed['education'] = self._extract_education_v7(lines)
        parsed['languages'], parsed['computer_skills'] = self._extract_skills_v7(lines)
        parsed['extra_curricular'] = self._extract_extra_curricular_v7(lines)
        
        return parsed
    
    def _extract_experience_v7(self, lines: List[str]) -> List[Dict[str, Any]]:
        """Extract experience with V7 logic - completely rewritten"""
        experience = []
        ...
        while i < len(lines):
            ...
            if in_experience_section and line:
                ...
                if self._is_company_line_v7(line):
                ...
                elif current_experience and self._is_job_title_line_v7(line):
                    # This is a job title
                    current_experience['title'] = line
                elif current_experience and self._is_date_line_v7(line):
                    # This contains dates - EXTRACT THEM PROPERLY
                    dates = self._extract_dates_from_line_v7(line)
                    if dates:
        ...
        for exp in experience:
            exp['responsibilities'] = self._rewrite_responsibilities_v7(exp.get('responsibilities', []))
        
        return experience
    
    def _is_company_line_v7(self, line: str) -> bool:
        """Check if line is a company name with V7 logic"""
    ...
    def _is_job_title_line_v7(self, line: str) -> bool:
        """Check if line is a job title with V7 logic"""
    ...
    def _is_date_line_v7(self, line: str) -> bool:
        """Check if line contains dates with V7 logic"""
    ...
    def _extract_dates_from_line_v7(self, line: str) -> str:
        """Extract dates from a line with V7 logic"""
    ...
    def _rewrite_responsibilities_v7(self, responsibilities: List[str]) -> List[str]:
        """Rewrite and clean up responsibilities with V7 logic"""
        rewritten = []
    ...
    def _extract_education_v7(self, lines: List[str]) -> List[Dict[str, Any]]:
        """Extract education with V7 logic - completely rewritten"""
        education = []
    ...
    def _extract_skills_v7(self, lines: List[str]) -> tuple[List[str], List[str]]:
        """Extract languages and computer skills with V7 logic - completely rewritten"""
        languages = []
        computer_skills = []
    ...
    def _extract_extra_curricular_v7(self, lines: List[str]) -> List[str]:
        """Extract extra curricular activities with V7 logic"""
        activities = []
    ...
    def _fill_template_with_data(self, template: str, data: Dict[str, Any], top_logo: str, bottom_logo: str) -> str:
        ...
        html = html.replace('{EXPERIENCE_ITEMS}', self._format_experience_items_v7(data))
        html = html.replace('{EDUCATION_ITEMS}', self._format_education_items_v7(data))
        html = html.replace('{SKILLS_LIST}', self._format_skills_v7_template(data))
    ...
    def _format_experience_items_v7(self, data: Dict[str, Any]) -> str:
        """Format experience items with V7 layout - DATES ON RIGHT SIDE"""
        items = []
    ...
    def _format_education_items_v7(self, data: Dict[str, Any]) -> str:
        """Format education items with V7 layout"""
        items = []
    ...
    def _format_skills_v7_template(self, data: Dict[str, Any]) -> str:
...
enhanced_cv_formatter_v7 = EnhancedCVFormatterV7()
