#!/usr/bin/env python3
"""
Enhanced CV Formatter V4 for Mawney Partners
Completely rewritten to handle the specific CV structure issues we're seeing
"""
...
class EnhancedCVFormatterV4:
    """Enhanced CV formatter V4 with specific fixes for the issues we're seeing"""
    ...
    def format_cv_with_template(self, cv_data: str, filename: str = '') -> Dict[str, Any]:
        """Format CV using the Mawney Partners template with specific fixes"""
        try:
            logger.info(f"=== ENHANCED CV FORMATTING V4 STARTED ===")
            logger.info(f"Filename: {filename}")
            logger.info(f"Raw CV data length: {len(cv_data)} characters")
            
            # Step 1: Clean and structure the CV text with specific fixes
            structured_cv = self._structure_cv_text_v4(cv_data)
            logger.info(f"Structured CV data length: {len(structured_cv)} characters")
            
            # Step 2: Parse CV with specific logic for the issues we're seeing
            parsed_data = self._parse_cv_v4(structured_cv)
            ...
            logger.info(f"=== ENHANCED CV FORMATTING V4 COMPLETED ===")
            
            return {
                'success': True,
                'html_version': formatted_html,
                'text_version': self._extract_text_from_html(formatted_html),
                'analysis': f"CV formatted using Mawney Partners template V4. Extracted: {len(parsed_data.get('experience', []))} experience items, {len(parsed_data.get('education', []))} education items.",
            ...
            logger.error(f"Error in enhanced CV formatting V4: {e}", exc_info=True)
            return {
                'success': False,
                'error': str(e),
                'html_version': '',
    ...
    def _structure_cv_text_v4(self, text: str) -> str:
        """Structure CV text with specific fixes for the issues we're seeing"""
        import re
    ...
    def _parse_cv_v4(self, cv_text: str) -> Dict[str, Any]:
        """Parse CV with specific fixes for the structure issues"""
        logger.info("Using CV parsing V4 with specific fixes...")
        ...
        phone_patterns = [
            ...
            r'\b\d{3}\s+\d{3}\s+\d{4}\b',
            r'\b\d{3}\s+\d{3}\s+\d{4}\b',  # For "783 6326056" format
            r'\b\d{3}\s+\d{7}\b'  # For "783 6326056" format
        ]
        ...
        # Parse sections with specific fixes
        parsed['experience'] = self._extract_experience_v4(lines)
        parsed['education'] = self._extract_education_v4(lines)
        parsed['languages'], parsed['computer_skills'] = self._extract_skills_v4(lines)
        parsed['extra_curricular'] = self._extract_extra_curricular_v4(lines)
        
        return parsed
    
    def _extract_experience_v4(self, lines: List[str]) -> List[Dict[str, Any]]:
        """Extract experience with specific fixes for the structure issues"""
        experience = []
        ...
        while i < len(lines):
            ...
            if in_experience_section and line:
                ...
                if self._is_company_line_v4(line):
                ...
                elif current_experience and self._is_job_title_line_v4(line):
                    # This is a job title
                    current_experience['title'] = line
                elif current_experience and self._is_date_line_v4(line):
                    # This contains dates
        ...
        for exp in experience:
            exp['responsibilities'] = self._rewrite_responsibilities_v4(exp.get('responsibilities', []))
        
        return experience
    
    def _is_company_line_v4(self, line: str) -> bool:
        """Check if line is a company name with specific logic"""
    ...
    def _is_job_title_line_v4(self, line: str) -> bool:
        """Check if line is a job title with specific logic"""
    ...
    def _is_date_line_v4(self, line: str) -> bool:
        """Check if line contains dates with specific logic"""
    ...
    def _rewrite_responsibilities_v4(self, responsibilities: List[str]) -> List[str]:
        """Rewrite and clean up responsibilities with specific fixes"""
        rewritten = []
    ...
    def _extract_education_v4(self, lines: List[str]) -> List[Dict[str, Any]]:
        """Extract education with specific fixes for the structure issues"""
        education = []
    ...
    def _extract_skills_v4(self, lines: List[str]) -> tuple[List[str], List[str]]:
        """Extract languages and computer skills with specific fixes"""
        languages = []
        computer_skills = []
    ...
    def _extract_extra_curricular_v4(self, lines: List[str]) -> List[str]:
        """Extract extra curricular activities with specific fixes"""
        activities = []
    ...
    def _fill_template_with_data(self, template: str, data: Dict[str, Any], top_logo: str, bottom_logo: str) -> str:
        ...
        html = html.replace('{EXPERIENCE_ITEMS}', self._format_experience_items_v4(data))
        html = html.replace('{EDUCATION_ITEMS}', self._format_education_items_v4(data))
        html = html.replace('{SKILLS_LIST}', self._format_skills_v4_template(data))
    ...
    def _format_experience_items_v4(self, data: Dict[str, Any]) -> str:
        """Format experience items with specific fixes"""
        items = []
    ...
    def _format_education_items_v4(self, data: Dict[str, Any]) -> str:
        """Format education items with specific fixes"""
        items = []
    ...
    def _format_skills_v4_template(self, data: Dict[str, Any]) -> str:
...
enhanced_cv_formatter_v4 = EnhancedCVFormatterV4()
