...
class EnhancedCVFormatterV19:
    def __init__(self):
        self.template_path = os.path.join(os.path.dirname(__file__), 'mawney_cv_template_separate_pages_v19.html')
        
    def format_cv_with_template(self, cv_content, filename):
        """Format CV using separate pages template with forced breaks"""
        try:
            ...
            # Format sections (keeping parsing exactly the same as V17/V18)
            work_exp = self._format_work_experience_v19(cv_data.get('work_experience', []))
            education = self._format_education_v19(cv_data.get('education', []))
            languages = self._format_languages_v19(cv_data.get('languages', []))
            computer_skills = self._format_computer_skills_v19(cv_data.get('computer_skills', []))
            extra_curricular = self._format_extra_curricular_v19(cv_data.get('extra_curricular', []))
            ...
            return {
                ...
                'filename': filename,
                'error': str(e),
                'version': 'V19_SeparatePages_Error'
            }
    ...
    def _parse_cv_data(self, text):
        ...
        if work_section:
            cv_data['work_experience'] = self._parse_work_experience_v19(work_section)
        else:
            # Try to find work experience without section header
            cv_data['work_experience'] = self._parse_work_experience_v19(text)
        
        # Parse education - improved
        education_section = self._extract_section(text, ['EDUCATION', 'ACADEMIC BACKGROUND', 'QUALIFICATIONS'])
        if education_section:
            cv_data['education'] = self._parse_education_v19(education_section)
        else:
            cv_data['education'] = self._parse_education_v19(text)
        
        # Parse languages
        languages_section = self._extract_section(text, ['LANGUAGES', 'LANGUAGE SKILLS'])
        if languages_section:
            cv_data['languages'] = self._parse_languages_v19(languages_section)
        ...
        if skills_section:
            cv_data['computer_skills'] = self._parse_computer_skills_v19(skills_section)
        
        # Parse extra curricular
        extra_section = self._extract_section(text, ['EXTRA CURRICULAR', 'ACTIVITIES', 'INTERESTS', 'HOBBIES'])
        if extra_section:
            cv_data['extra_curricular'] = self._parse_extra_curricular_v19(extra_section)
    ...
    def _parse_work_experience_v19(self, section):
        """Improved work experience parsing - same as V17/V18"""
        entries = []
    ...
    def _parse_education_v19(self, section):
        """Improved education parsing - same as V17/V18"""
        entries = []
    ...
    def _parse_languages_v19(self, section):
        """Parse languages - same as V17/V18"""
        languages = []
    ...
    def _parse_computer_skills_v19(self, section):
        """Parse computer skills - same as V17/V18"""
        skills = []
    ...
    def _parse_extra_curricular_v19(self, section):
        """Parse extra curricular activities - same as V17/V18"""
        activities = []
    ...
    def _format_work_experience_v19(self, work_exp):
        """Format work experience with table layout for proper date alignment"""
        if not work_exp:
    ...
    def _format_education_v19(self, education):
        """Format education with table layout for proper date alignment"""
        if not education:
    ...
    def _format_languages_v19(self, languages):
    ...
    def _format_computer_skills_v19(self, skills):
    ...
    def _format_extra_curricular_v19(self, activities):
...
# Create instance
enhanced_cv_formatter_v19 = EnhancedCVFormatterV19()
