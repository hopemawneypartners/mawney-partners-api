#!/usr/bin/env python3
"""
Enhanced CV Formatter V5 for Mawney Partners
Final version to fix all remaining issues from the screenshot
"""
...
class EnhancedCVFormatterV5:
    """Enhanced CV formatter V5 - Final fixes for all remaining issues"""
    ...
    def format_cv_with_template(self, cv_data: str, filename: str = '') -> Dict[str, Any]:
        """Format CV using the Mawney Partners template with final fixes"""
        try:
            logger.info(f"=== ENHANCED CV FORMATTING V5 STARTED ===")
            logger.info(f"Filename: {filename}")
            logger.info(f"Raw CV data length: {len(cv_data)} characters")
            
            # Step 1: Aggressive text cleaning and structuring
            structured_cv = self._aggressive_text_cleaning(cv_data)
            logger.info(f"Cleaned CV data length: {len(structured_cv)} characters")
            
            # Step 2: Parse CV with final logic
            parsed_data = self._parse_cv_final(structured_cv)
            ...
            logger.info(f"=== ENHANCED CV FORMATTING V5 COMPLETED ===")
            
            return {
                'success': True,
                'html_version': formatted_html,
                'text_version': self._extract_text_from_html(formatted_html),
                'analysis': f"CV formatted using Mawney Partners template V5. Extracted: {len(parsed_data.get('experience', []))} experience items, {len(parsed_data.get('education', []))} education items.",
            ...
            logger.error(f"Error in enhanced CV formatting V5: {e}", exc_info=True)
            return {
                'success': False,
                'error': str(e),
                'html_version': '',
    ...
    def _parse_cv_final(self, cv_text: str) -> Dict[str, Any]:
        """Parse CV with final logic to handle all the issues"""
        logger.info("Using CV parsing V5 with final fixes...")
        ...
        # Parse sections with final logic
        parsed['experience'] = self._extract_experience_final(lines)
        parsed['education'] = self._extract_education_final(lines)
        parsed['languages'], parsed['computer_skills'] = self._extract_skills_final(lines)
        parsed['extra_curricular'] = self._extract_extra_curricular_final(lines)
        
        return parsed
    
    def _extract_experience_final(self, lines: List[str]) -> List[Dict[str, Any]]:
        """Extract experience with final logic to fix date positioning"""
        experience = []
        ...
        while i < len(lines):
            ...
            if in_experience_section and line:
                ...
                if self._is_company_line_final(line):
                ...
                elif current_experience and self._is_job_title_line_final(line):
                    # This is a job title
                    current_experience['title'] = line
                elif current_experience and self._is_date_line_final(line):
        ...
        for exp in experience:
            exp['responsibilities'] = self._rewrite_responsibilities_final(exp.get('responsibilities', []))
        
        return experience
    
    def _is_company_line_final(self, line: str) -> bool:
        """Check if line is a company name with final logic"""
    ...
    def _is_job_title_line_final(self, line: str) -> bool:
        """Check if line is a job title with final logic"""
    ...
    def _is_date_line_final(self, line: str) -> bool:
        """Check if line contains dates with final logic"""
    ...
    def _extract_dates_from_line(self, line: str) -> str:
        """Extract dates from a line with final logic"""
    ...
    def _rewrite_responsibilities_final(self, responsibilities: List[str]) -> List[str]:
        """Rewrite and clean up responsibilities with final logic"""
        rewritten = []
    ...
    def _extract_education_final(self, lines: List[str]) -> List[Dict[str, Any]]:
        """Extract education with final logic to handle the messy content we're seeing"""
        education = []
    ...
    def _extract_skills_final(self, lines: List[str]) -> tuple[List[str], List[str]]:
        """Extract languages and computer skills with final logic"""
        languages = []
        computer_skills = []
        ...
        if not languages and not computer_skills and in_skills_section:
            ...
            for line in skills_lines:
                if any(word in line.lower() for word in ['financial', 'risk', 'statistical', 'mathematics', 'modelling', 'analysis']):
                    computer_skills.append(line)
                elif any(word in line.lower() for word in ['english', 'french', 'spanish', 'german', 'italian', 'chinese', 'japanese', 'arabic', 'russian', 'portuguese']):
    ...
    def _extract_extra_curricular_final(self, lines: List[str]) -> List[str]:
        """Extract extra curricular activities with final logic"""
        activities = []
    ...
    def _fill_template_with_data(self, template: str, data: Dict[str, Any], top_logo: str, bottom_logo: str) -> str:
        ...
        html = html.replace('{EXPERIENCE_ITEMS}', self._format_experience_items_final(data))
        html = html.replace('{EDUCATION_ITEMS}', self._format_education_items_final(data))
        html = html.replace('{SKILLS_LIST}', self._format_skills_final_template(data))
    ...
    def _format_experience_items_final(self, data: Dict[str, Any]) -> str:
        """Format experience items with final layout - DATES ON RIGHT SIDE"""
        items = []
    ...
    def _format_education_items_final(self, data: Dict[str, Any]) -> str:
        """Format education items with final layout"""
        items = []
    ...
    def _format_skills_final_template(self, data: Dict[str, Any]) -> str:
...
enhanced_cv_formatter_v5 = EnhancedCVFormatterV5()
